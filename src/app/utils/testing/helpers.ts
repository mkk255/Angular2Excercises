 ...
 */
export function makeCreateTestComponent(getClass: ComponentClazzFn, detectChanges = true) {
  ...
  return function createTestComponent(template?: string, styles?: any): ComponentFixture<Type<any>> {
    ...
    return !template ? createFixture(componentAny,detectChanges) : TestBed
...
export function createFixture(component:Type<any>, detectChanges = true): ComponentFixture<Type<any>> {
 ...
 */
export function makeDOMQueryForExpect(getClass: ComponentClazzFn) {
  let createTestComponent;

  // Return actual `expectTemplate()` function
  return function expectDomForQuery(selector: string, template: string, index = 0): any {
    if (!createTestComponent) {
      createTestComponent = makeCreateTestComponent(getClass);
    }
